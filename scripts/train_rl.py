...
def _run_training_loop(
    ...
    """Run the epoch training loop with evaluation, visuals, and checkpointing.

    Args:
        trainer: Initialized PGNRLTrainer.
        cfg: Full PGN RL config.
        pgn_path: Path to the PGN file used for training and evaluation.
        ckpt_path: Where to save per-epoch checkpoints, or None to skip.
        tracker: Metric tracker for logging to Aim (or no-op).
    """
    try:
        for epoch in range(1, cfg.rl.epochs + 1):
            ...
            # Guard against silent key collisions before merging dicts.
            overlap = metrics.keys() & val_metrics.keys()
            if overlap:
                raise ValueError(
                    f"Metric key collision between train and val: {overlap}"
                )
            ...
            if _MPL_AVAILABLE:
                try:
                    figs = trainer.sample_visuals(
                        ...
                        train_accuracy=val_metrics.get("val_accuracy"),
                    )
                    ...
                    logger.warning(
                        "sample_visuals failed at epoch %d",
                        epoch,
                        exc_info=True,
                    )
        ...
        if ckpt_path is not None:
            ...
            logger.info(
                "Emergency checkpoint saved: %s", emergency_ckpt_path
            )
        raise
...
def main() -> None:
    ...
    ckpt_path: Path | None = (
        Path(cfg.rl.checkpoint) if cfg.rl.checkpoint else None
    )
    if ckpt_path is not None:
        ckpt_path.parent.mkdir(parents=True, exist_ok=True)

    total_steps = cfg.rl.epochs * max(cfg.data.max_games, 1)
    ...
    try:
        ...
        _run_training_loop(trainer, cfg, pgn_path, ckpt_path, tracker)
    finally:
