"""Offline RL v2 training script (outcome-only, no value head).

Usage:
    python -m scripts.train_rl_v2 --config configs/train_rl_v2.yaml
"""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        description=(
            "Offline RL v2 training on PGN master games "
            "(outcome-only, no value head)."
        )
    )
    p.add_argument(
        "--config",
...
def _run_training_loop(
    ...
    Args:
        trainer: Initialized PGNRLTrainerV2.
...
def main() -> None:
    """Entry point for offline RL v2 PGN training."""
    logging.basicConfig(
        level=logging.INFO,
        format=(
